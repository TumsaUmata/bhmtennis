...
const doublesCategory = categories.find((c) => c.id === "mixed-doubles")!;
const totalMatches = (doublesCategory.groupSize * (doublesCategory.groupSize - 1)) / 2;

export default function MixedDoublesPage() {
  ...
  const [serviceTeamIds, setServiceTeamIds] = useState<string[]>([]);
  const [categoryData, setCategoryData] = useState<Category>(doublesCategory);
  ...
  const refresh = useCallback(async () => {
    const [allMatches, standings, nameMap, cat] = await Promise.all([
      service.getMatches("mixed-doubles"),
      service.getStandings("mixed-doubles", "A"),
      service.getPlayerNameMap("mixed-doubles", "A"),
      service.getCategory("mixed-doubles"),
    ]);
    setServiceMatches(allMatches);
    setServiceStandings(standings);
    setServicePlayerNames(nameMap);
    setServiceTeamIds([...nameMap.keys()]);
  ...
  const teamIds = serviceTeamIds;
  const matches = serviceMatches;
  const standings = serviceStandings;
  const remainingFixtures = getRemainingFixtures(teamIds, matches);
  const completedMatches = matches.filter((m) => m.status === "completed").length;
  const progress = (completedMatches / totalMatches) * 100;
  const groupComplete = completedMatches >= totalMatches;
  const adminClosed = store.closedGroups.has("mixed-doubles");
  const shouldLock = groupComplete || adminClosed;

  const lockedData = store.lockedBrackets.get("mixed-doubles");
  const isLocked = !!lockedData;
  if (shouldLock && !isLocked) {
    const finalBracket = generateFinalBracket(standings, "Mixed Doubles", false);
    store.lockBracket("mixed-doubles", finalBracket);
  }

  const bracket = isLocked ? advanceBracket(lockedData!.bracket) : generateFinalBracket(standings, "Mixed Doubles", true);
  ...
  const lockedDataEarly = store.lockedBrackets.get("mixed-doubles");
  const adminClosedEarly = store.closedGroups.has("mixed-doubles");
  ...
  const handleSubmit = useCallback(async (sets: SetScore[], winnerId: string) => {
    if (resumingMatch) {
      ...
      await service.addMatch("mixed-doubles", "A", selectedFixture.p1, selectedFixture.p2, sets, winnerId);
      store.addMatch("mixed-doubles", "A", selectedFixture.p1, selectedFixture.p2, sets, winnerId);
  ...
  const handleSaveIncomplete = useCallback(async (sets: SetScore[]) => {
    if (resumingMatch) {
      ...
      await service.saveIncompleteMatch("mixed-doubles", "A", selectedFixture.p1, selectedFixture.p2, sets);
      store.saveIncompleteMatch("mixed-doubles", "A", selectedFixture.p1, selectedFixture.p2, sets);
      setSelectedFixture(null);
    }
    await refresh();
    forceUpdate((n) => n + 1);
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          <h1 className="text-2xl font-bold tracking-tight">Mixed Doubles</h1>
          <p className="text-sm text-muted-foreground">6 teams round-robin — Top 2 advance to final</p>
        </div>
      ...
      <Tabs value={view} onValueChange={(v) => { setView(v as "group" | "final"); handleCancel(); }}>
        ...
        <TabsContent value="group" className="space-y-4 mt-4">
          <div className="flex items-center justify-between text-sm">
            ...
            <span className="text-xs font-medium">Deadline: {new Date(doublesCategory.groupDeadline).toLocaleDateString("en-GB", { day: "numeric", month: "short" })}</span>
          </div>
          <Progress value={progress} className="h-1.5" />

          <Card>
            <CardHeader className="pb-2"><CardTitle className="text-sm font-semibold">Standings</CardTitle></CardHeader>
            <CardContent><StandingsTable standings={standings} advancementSlots={doublesCategory.advancementSlots} /></CardContent>
          </Card>
